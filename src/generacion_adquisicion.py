...
if __name__ == "__main__":
    ...
    # Adquisición y reproducción
    def devices():
        label = Label(window, text=str(sd.query_devices()))
        label.pack()
    ...
    # Interfaz gráfica   
    window = Tk()
    window.title("Generación y adquisición de datos")

    input_device = Entry(window, width=100)
    input_device.insert(0, "Ingrese el dispositivo de entrada: ")
    input_device.pack()
    output_device = Entry(window, width=100)
    output_device.insert(0, "Ingrese el dispositivo de salida: ") 
    output_device.pack()

    def play_record_sweep():
        signal = 'SineSweepLog.wav'
        record_signal(signal, int(input_device.get()), int(output_device.get()))
        signal_rec, fs = read_wav('record_{}'.format(signal))
        time_domain_plot(signal_rec, fs)

    button = Button(window, 
                    text="Graficar Ruido Rosa en dominio del tiempo", 
                    command=plot_pink, 
                    font=("Arial", 16),
                    fg="blue")
    button2 = Button(window, 
                    text="Graficar Sine Sweep y Filtro inverso en dominio del tiempo", 
                    command=plot_swep_inv, 
                    font=("Arial", 16),
                    fg="blue")
    button3 = Button(window, 
                    text="Ver dispositivos de audio", 
                    command=devices, 
                    font=("Arial", 16),
                    fg="blue")
    button4 = Button(window, 
                    text="Reproducir y Grabar Sine Sweep", 
                    command=play_record_sweep, 
                    font=("Arial", 16),
                    fg="blue")
