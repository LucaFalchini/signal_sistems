...
def esc_log(data_impulse, a = 20):
    ...
    data_impulse, fs = sf.read("impulso_aula2.wav")   
...
def analisis_frecuencias(audio, fs):
    ...
    Parametros
    ----------
    audio: Numpy array con los datos del audio

    fs: int, Frecuencia de muestreo.
      
    return: Numpy array 
        Datos en el dominio frecuencial.
...
if __name__ == '__main__':
    ...
    def play():
        reproducir(file)
    def esc():
        data_log = esc_log(data)
        time_domain_plot(data_log, fs, graph_name="impulso escala logarítmica")
    def freq():
        analisis_frecuencias(data, fs)
    
    
    # Interfaz gráfica   
    window = Tk()
    window.title("Funciones")

    button = Button(window, 
                    text="Graficar en dominio del tiempo", 
                    command=graph_1, 
                    font=("Arial", 16),
                    fg="blue")
    ...
    button3 = Button(window, 
                    text="Graficar en escala logarítmica", 
                    command=esc, 
                    font=("Arial", 16),
                    fg="blue")
    button4 = Button(window, 
                    text="Graficar dominio de la frecuencia", 
                    command=freq, 
                    font=("Arial", 16),
                    fg="blue")
